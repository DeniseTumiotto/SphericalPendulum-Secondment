...
loglog(time_vec, time_vec*(R_err(end)/time_vec(end)), 'y-', 'LineWidth', 1, 'DisplayName','1^{st} order')
loglog(time_vec, time_vec.^2*(R_err(end)/time_vec(end)^2), 'g-', 'LineWidth', 1, 'DisplayName','2^{nd} order')
